...
import (
	...
	federation "github.com/kubernetes-sigs/federation-v2/pkg/client/clientset_generated/clientset"
...
const (
	...
	informerSyncPeriod = 30 * time.Second
)
...
type ServiceDNSController struct {
	...
	fedClient      federation.Interface
	dns            dnsprovider.Interface
	federationName string
	// dnsSuffix is the DNS suffix we use when publishing service DNS names
	dnsSuffix string
	...
	dnsObjectStore corelisters.ServiceLister
...
// NewDNSController returns a new service dns controller to manage DNS records for federated services
func NewDNSController(client federation.Interface, dnsProvider, dnsProviderConfig, federationName,
	dnsSuffix, zoneName, zoneID string) (*ServiceDNSController, error) {
	...
	serviceIndexer, d.dnsObjectController = cache.NewIndexerInformer(
		&cache.ListWatch{
	...
	d.dnsObjectStore = corelisters.NewServiceLister(serviceIndexer)

	return d, nil
}

func (s *ServiceDNSController) Run(workers int, stopCh <-chan struct{}) {
...
func (s *ServiceDNSController) deliverService(dnsObject *feddnsv1a1.MultiClusterDNSLb, delay time.Duration, failed bool) {
...
func (s *ServiceDNSController) workerFunction() bool {
	...
	for _, clusterDNS := range dnsObject.Status.DNS {
		...
		if err != nil {
			...
			s.deliverService(dnsObject, 0, true)
		}
	}
	return false
}
...
func (s *ServiceDNSController) getHealthyEndpoints(cluster, zone, region string, dnsObject *feddnsv1a1.MultiClusterDNSLb) (zoneEndpoints, regionEndpoints, globalEndpoints []string) {
...
func (s *ServiceDNSController) ensureDNSRecords(cluster, zone, region string, dnsObject *feddnsv1a1.MultiClusterDNSLb) error {
	// Quinton: Pseudocode....
